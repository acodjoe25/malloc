...
static inline int block_end_allocated(block_t *b) {
    // change pls
    // int test = (int)b->payload[b->size/8 - 2];

    return (int)(b->payload[b->size / 8 - 2] & 1);
}
...
static inline void block_set_size(block_t *b, long size) {
    ...
    // an int representing if the block is allocated
    int aloc = block_allocated(b);

    // the size combined with if its allocate
    long new_size = size | aloc;

    // setting the end and begining block
    b->size = new_size;
    b->payload[b->size / 8 - 2] = new_size;
}
 ...
 * from the size
 */
static inline long block_size(block_t *b) { return b->size & -2; }
 ...
 */
static inline long block_prev_size(block_t *b) { return b->payload[-2] & -2; }
...
static inline block_t *block_prev(block_t *b) {
    return (block_t *)&b->payload[-1 - (block_prev_size(b) / WORD_SIZE)];
}
